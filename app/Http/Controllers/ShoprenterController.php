...
class ShoprenterController extends Controller
{
	...
	public function checkZip(Request $request)
	{
		...
		if ($reseller) {
			...
			if ($rZip->region->biobubi_delivery) {
				return response()->json([
					'success' => true,
					'found'   => true,
					'message' => 'Irányítószám megtalálva és rendelkezik BioBubi futárral.',
				]);
			} else {
				return response()->json([
					'success' => true,
					'found'   => false,
					'message' => 'Irányítószám megtalálva de nem rendelkezik BioBubi futárral.',
				]);
			}
		} else {
