...
@section('content')
    <div class="container">
        ...
        <div class="card card-body">
            <form action="{{ action('BundleController@update', $bundle->sku) }}" method="post">
                @csrf
                @method('put')
                <div class="form-group">
                    ...
                    <input type="text" name="bundle-sku" id="bundle-sku" class="form-control" value="{{ $bundle->name }}" disabled="disabled" readonly="readonly">
                </div>

                <hr class="my-4">

                <div id="bundle-product-rows-container">
                    @php /** @var \App\Product $bundle */ @endphp
                    @php /** @var \App\BundleProduct $subProduct */ @endphp
                    @foreach($bundle->subProducts as $i => $subProduct)
                        <div class="bundle-product-row form-row align-items-end mb-3" id="bundle-product-row-{{ $i }}">
                            <div class="col">
                                <div class="form-group mb-0">
                                    <label for="bundle-product-sku[{{ $i }}]">Rész termék *</label>
                                    <select name="bundle-product-sku[{{ $i }}]" id="bundle-product-sku[{{ $i }}]" class="custom-select" required>
                                        @foreach($products as $product)
                                            <option value="{{ $product->sku }}" @if($subProduct->product_sku == $product->sku) selected @endif>{{ $product->name }} ({{ $product->sku }})</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-2">
                                <div class="from-group mb-0">
                                    <label for="bundle-product-count[{{ $i }}]" class="w-100 text-right">Darabszám *</label>
                                    <div class="input-group">
                                        <input type="tel" id="bundle-product-count[{{ $i }}]" name="bundle-product-count[{{ $i }}]" class="input-count form-control text-right" value="{{ old('bundle-product[' . $i . ']', $subProduct->product_qty) }}" required>
