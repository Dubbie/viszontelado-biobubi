@if($user->reports()->count() > 0)
        ...
        <div class="col-md-auto">
            <form action="">
                <div class="form-group">
                    <label for="date" class="sr-only">Havi riport</label>
                    <select name="date" id="date" class="custom-select">
                        @foreach($user->reports()->orderByDesc('created_at')->get() as $report)
                            <option value="{{ $report->created_at->format('Y-m') }}"
                                    @if($selectedReport && $selectedReport->id == $report->id) selected @endif>{{ \Illuminate\Support\Str::title($report->created_at->translatedFormat('Y F')) }}</option>
                        @endforeach
                    </select>
                </div>
            </form>
            <button class="btn btn-primary btn-block" id="btn-yearly-view">Éves nézet</button>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card card-body border shadow-none mb-4 mb-md-0">
                    ...
                    @foreach($report->reportProducts as $rp)
                        <div class="row no-gutters">
                            <div class="col-auto text-right mr-2">
                                <p class="mb-0"><small class="font-weight-bold">{{ $rp->product_qty }} db</small></p>
                            </div>
                            <div class="col">{{ $rp->product->name }}</div>
                        </div>
                    @endforeach
                @else
