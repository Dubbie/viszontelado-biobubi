                ...
                <div class="card card-body mt-4">
                    ...
                    <form action="{{ action('OrderCommentController@store') }}" method="POST" class="mt-4">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="comment-content" id="comment-content" cols="30" rows="6" class="form-control" required></textarea>
                        </div>
                    ...
                    <div id="order-comments" class="list-group">
                        @if($localOrder->todos->count() > 0)
                            ...
                            @foreach($localOrder->todos()->orderBy('completed_at')->orderBy('deadline')->get() as $todo)
                                <div class="list-group-item order-todo d-flex p-3 @if($todo->isCompleted()) todo-complete @endif">
                                    ...
                                    <div class="details w-100">
                                        <div class="row">
                                            <div class="col">
                                                @if($todo->isCompleted())
                                                    <p class="lead font-weight-bold mb-3 text-muted" style="text-decoration: line-through;">{{ $todo->content }}</p>
                                                @else
                                        ...
                                        @if(!$todo->isCompleted())
                                            <p class="text-small mb-0 row">
                                                ...
                                                <span class="col text-right font-weight-normal">{{ $todo->deadline->format('Y.m.d H:i:s') }}</span>
                                            </p>
                                        @else
                                            <p class="text-small mb-0 row">
                                                <span class="col font-weight-bold">Teljesítve:</span>
                                                <span class="col text-right font-weight-normal">{{ $todo->completed_at->format('Y.m.d H:i:s') }}</span>
                                            </p>
                                        @endif
